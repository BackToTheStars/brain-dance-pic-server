...
Ручка админской таблицы. К типу медиа не привязана — тип здесь фильтр,
...
Считается только начало просмотра — запрос без `Range` либо с `Range`,
...
не удаляются: какая из них лишняя — отдельный вопрос.
