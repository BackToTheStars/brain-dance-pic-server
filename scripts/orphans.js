// Поиск файлов в GridFS, на которые нет записи Media (BP-9). Через API такой
...
// начиная с BP-5, поэтому здесь не разбирается.
//
...
async function scanType(db, type) {
  ...
  // среди них — не сироты, но и молчать о них нельзя (BP-13).
  ...
  // такая запись снимается обычным DELETE /<тип>/:id начиная с BP-5. Считается
...
function printReport(results) {
  ...
  for (const result of results) {
    ...
    if (result.duplicates.length) {
      ...
      // отдельный вопрос (BP-13), поэтому скрипт их только показывает.
      console.log(
    ...
    if (result.withoutFile.length) {
      ...
      console.log(
        `  записей Media без файла: ${result.withoutFile.length} — ` +
          `это обратный случай, снимается DELETE /${result.type}/:id (BP-5); ` +
